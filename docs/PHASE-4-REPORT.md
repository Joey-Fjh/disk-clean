...
| 鉴权失败且扫描结果保留 | 通过 |
| 失败与重试 UI | 通过 |
| 成功分析 UI（本地 Mock Provider） | 通过（含 30 MB 单位复验与 Analyzer-only 安全文案） |

阶段 4 **已完成**。

### 收尾修复（2026-08-26）

- 统一 `formatBytes`：修复 evidence 中 30 MB 误显示为 30 GB
- Analyzer-only + Agent 建议：更新不可选文案，不扩大清理授权
- 失败横幅补充「本地规则建议仍可使用」
- `CONTENT_TYPE_LABELS.agent` 改为「AI 工具缓存」

### 非阻断 UI 文案观察（已处理 / 记录）

1. ~~智能分析失败时，可补充「本地规则建议仍可使用」类提示。~~（已补充）
2. ~~候选项内容类型「AI / Agent」易被理解为判断来源。~~（已改为「AI 工具缓存」）
...
- [x] 扫描完成 →「Agent 正在分析」→「智能建议已生成」
- [x] 无 Key 时显示「未配置模型…」+ 前往设置
- [x] 每条候选项展示 Agent 建议、理由、影响、置信度、来源
- [x] 失败后可「重试分析」，扫描结果不丢失
- [x] 停止扫描后显示「未运行智能分析」，不产生模型请求
- [x] 增量扫描批次不重复请求模型
- [x] 分析后分类/合法勾选/滚动位置保持
- [x] Agent keep 后不可选项不再显示为已勾选
- [x] 无「自动删除」类文案
- [x] 1366×768 / 1920×1080 无嵌套滚动
